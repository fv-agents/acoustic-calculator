...
### Nieuw sinds 21-07
- **Echte productafmetingen (W×L×H)**: 79 producten hebben nu een bevestigde `dim`-string (van in-zee.nl gescraped + Falco's handmatige aanvulling voor Nova/Blaze/Orbit/Wing/Bold), zichtbaar in de stap-3 specs-modal en de PDF-fixturekaarten. Gevonden discrepanties (Sliced 800→Ø660, Blaze 550→Ø350, Cloud round 1200→Ø1190) bewust niet stilzwijgend gecorrigeerd in het `d`-veld — zie session-log 21-07 (12). Nog open: Flora-afmetingen (komt van Falco), een paar wall/XL-varianten onbevestigd.
...
- **Stap 4 — fotogrid + Netlify-functie**: header omgedraaid (project/datum links, groter logo rechts), nieuwe "Product details"-sectie (3 kaarten/rij, foto + specs) onder de fixture-tabel. Foto's lopen via een nieuwe Netlify Function (`netlify/functions/image-proxy.js`, whitelisted op lumenear.com) omdat lumenear.com geen CORS-headers stuurt — eerste server-side onderdeel van deze verder statische site. Zie session-log 21-07 (10).
